...
export class POWER implements IPOWER {
  getUpdatedUrl(params: POWERApiParams): Promise<any> {    
    let url = this.getApiUrl(params.resolution);

      //change long 
      url = this.changeParamters(params.long,url,'longitude');
      //change lat
      url = this.changeParamters(params.lat,url,'latitude');
      //change start 
      url = this.changeParamters(params.from,url,'start');
      //chnage end
      url = this.changeParamters(params.to,url,'end');
    ...
    getApiUrl(time:TimeResolution) {      
      let arr={
        "hourly":"/api/temporal/hourly/point?parameters=T2M&community=SB&longitude=0&latitude=0&start=20170101&end=20170102&format=JSON",
        "daily":"/api/temporal/daily/point?parameters=T2M&community=SB&longitude=0&latitude=0&start=20170101&end=20170201&format=JSON",
        "weekly":"",
        "monthly":"/api/temporal/monthly/point?parameters=T2M,T2M_MAX&community=SB&longitude=0&latitude=0&format=JSON&start=2016&end=2017"
      }
      return arr[time];
    
  }
  changeParamters(value,url:string,paramName:string){
    let val=value.toString(); //input can be dates,string,....
        if(val.length == 0)   //if no parameter passed return
            return;
